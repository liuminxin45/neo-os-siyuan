...
const checks = [
  ...
  ["paginated read signature", "src/services/agent-runtime.ts", "page=${page || \"1\"}"],
  ["mutation repeat guard continues", "src/services/agent-runtime.ts", "mutationRequirementMessage(userGoal, guardReason)"],
  ["mutation mismatch fallback", "src/services/agent-runtime.ts", "如果 skill 主流程与用户目标不完全匹配"],
  ["inventory args avoids help", "src/services/agent-runtime.ts", "suggestedInventoryArgs"],
  ["skill inventory intent", "src/services/agent-runtime.ts", "isSkillInventoryIntent"],
  ["skill inventory direct path", "src/services/agent-runtime.ts", "path: \"/LLM-Wiki/skills\""],
  ["skill inventory root rewrite", "src/services/agent-runtime.ts", "normalizeSkillInventoryArgs"],
  ["skill listing not shallow root", "src/services/agent-runtime.ts", "isFinalizableSkillListingAction"],
  ...
  ["skill listing prompt requires skills path", "src/adapters/llm-chat-completions.ts", "只看到 /LLM-Wiki 下有 skills 目录和 children 数量还不足以最终回答"],
  ...
  ["skill write mismatch prompt", "src/adapters/llm-chat-completions.ts", "不要停在让用户选择 A/B/C"],
  ["skill write completion guard", "src/services/agent-runtime.ts", "requiresSuccessfulMutation"],
  ["auto ingest avoids stale sources path", "src/adapters/llm-chat-completions.ts", "不要使用不存在的 /LLM-Wiki/raw/sources/auto-ingest/"],
  ["auto ingest path normalization", "src/services/agent-runtime.ts", "normalizeAutoIngestWriteArgs"],
  ["auto ingest career route", "src/services/chat-service.ts", "/LLM-Wiki/raw/experience/<slug>"],
