...
const toWireMessages = (
  ...
  const toolHint = toolNames
    ? `当前可调用的 MCP tools.name：${toolNames}。\n可用工具契约摘要：\n${toolContracts}\n本插件主要用于思源笔记工作区，用户即使没有明确说“使用 MCP”，只要问题涉及笔记本、文档、块、数据库、标签、文件、搜索、当前工作区内容、笔记内容或需要查询/读取/遍历/统计思源里的信息，就应优先查看可用工具的 description 和 input_schema，自主选择合适 MCP 工具并用结构化工具调用协议执行。${skillInstruction}${mutationLedgerInstruction}${documentLinkInstruction}工具的 input_schema 已经在结构化 tools 定义和上方契约摘要中给出；除非用户明确询问工具用法，否则不要先调用 help。调用工具前必须根据已知 schema 选择具体 action 和必填字段；不要用空参数调用需要 action 或查询参数的工具。每轮调用后必须阅读 ReAct 历史里的 Observation 并基于结果继续；如果上一轮 help 已返回用法，下一轮不要重复 help，要选择具体 action 和参数。如果已经拿到 notebook list、fs.ls、fs.tree、get_child_docs 或 search_docs 的有效结果，且足以回答用户“有哪些/列出/查看文档”的问题，应直接总结当前可见范围和受限项，不要继续展开全量深层目录。notebook.list 只能证明笔记本存在，不能证明内容可读；fs.ls "/" 才代表当前 MCP 文件接口可读根，permission_denied 必须写成访问受限。如果用户说“这篇”“那篇”“上一篇”等指代，而前文或 Observation 已经出现明确 path，应优先 read 该 path，不要重新泛搜关键词。只有纯常识、写作、解释或无需工作区数据的问题才直接回答。不要在正文中输出 XML、伪标签或未出现在 tools.name 中的工具名。`
...
const toAnthropicWireMessages = (
  ...
  if (system?.role === "system") {
    const toolNames = tools.map((tool) => tool.llmName).join(", ");
    const toolHint = toolNames
      ? `\n当前可用 MCP 工具名必须完全按 tools.name 使用：${toolNames}。请根据每个工具的 description 和 input_schema 自行判断用户意图、选择工具与参数。用户没有明确说 MCP 时，只要问题需要思源工作区数据，也要优先调用合适工具。成功执行任何写入、编辑、删除、移动、创建、重命名或属性更新类调用后，最终回答必须说明实际变更；插件会自动追加固定格式的“实际变更清单”。如果已经找到或引用 LLM-Wiki 文档，最终回答应使用 [[文档标题]] 或 [文档标题](wiki/...) 形式，让插件渲染为可点击入口。工具 schema 已随 tools 定义提供；除非用户明确询问工具用法，否则不要先调用 help。调用前必须根据 action enum 和必填字段选择具体参数，不要用空参数调用需要 action 或查询参数的工具。每轮调用后必须阅读 ReAct 历史里的 Observation 并基于结果继续；如果上一轮 help 已返回用法，下一轮不要重复 help，要选择具体 action 和参数。如果已经拿到 notebook list、fs.ls、fs.tree、get_child_docs 或 search_docs 的有效结果，且足以回答用户“有哪些/列出/查看文档”的问题，应直接总结当前可见范围和受限项，不要继续展开全量深层目录。notebook.list 只能证明笔记本存在，不能证明内容可读；fs.ls "/" 才代表当前 MCP 文件接口可读根，permission_denied 必须写成访问受限。如果用户说“这篇”“那篇”“上一篇”等指代，而前文或 Observation 已经出现明确 path，应优先 read 该 path，不要重新泛搜关键词。不要编造未出现在 tools.name 中的工具名。`
      : "";
